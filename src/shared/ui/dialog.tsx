  ...
  ({ className, children, ...props }, ref) => (
    <DialogPortal>
      ...
      <DialogPrimitive.Content
        ref={ref}
        className={cn(
          'fixed left-1/2 top-1/2 z-50 grid w-[min(32rem,90vw)] max-h-[90vh] translate-x-[-50%] translate-y-[-50%] gap-[var(--space-md)] rounded-2xl border border-border bg-background p-[var(--space-lg)] shadow-2xl',
...
const DialogHeader = ({ className, ...props }: React.ComponentProps<'div'>) => (
  <div className={cn('flex flex-col gap-[var(--space-xs)] text-center sm:text-left', className)} {...props} />
);
...
const DialogFooter = ({ className, ...props }: React.ComponentProps<'div'>) => (
  <div className={cn('flex flex-col-reverse gap-[var(--space-sm)] sm:flex-row sm:justify-end', className)} {...props} />
);
